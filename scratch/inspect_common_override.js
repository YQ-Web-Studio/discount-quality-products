...
add_action( 'rest_api_init', function () {
  ...
  const snippetPayload = {
    name: 'Antigravity Inspect CommonOverride',
    code: snippetCode,
    active: 1,
    scope: 'global'
  };
  ...
  try {
    ...
    console.log('Querying debug-common-override...');
    const queryRes = await apiRequest('/wp-json/dqp/v1/debug-common-override', 'GET');
    console.log('Query Status:', queryRes.statusCode);

    if (queryRes.statusCode === 200) {
      const parsed = JSON.parse(queryRes.body);
      console.log('Hooks:', parsed.hooks);
